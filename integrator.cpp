...
namespace fst
{
    ...
    math::Vector3f Integrator::renderPixel(const Ray& ray, int depth) const
    {
        ...
        for (auto& light : m_scene.point_lights)
        {
            ...
            {
                if(hit_record.decal_mode == "replace_all")
                    color = hit_record.texture_color;
                else
                    color = color + light.computeRadiance(light_pos_distance) * material.computeBrdf(to_light, -ray.get_direction(), hit_record.normal, hit_record);
            }
        }
